      ...
      {data.map((work) => (
        <div key={work.id} className={styles.arrayGroupCard}>
          ...
          <div className={styles.inputGroup}>
            ...
            <textarea className={styles.inputField} value={work.workDesc} onChange={(e) => onChange(work.id, 'workDesc', e.target.value)} placeholder="Developed backend modules..." />
          </div>
        </div>
      ))}
      <button type="button" className={styles.btnAddItem} onClick={onAdd}>➕ Append Experience Node</button>
    </>
  );
};
